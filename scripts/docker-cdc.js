...
program
  .version('1.0.0')
  .description('A utility for managing ClrAddons css regression with gemini')
...
console.log('ClrAddons Docker Control for Gemini CSS Regression Testing');
console.log('--------------------------------------------------------');
console.log();
 ...
 */
if (program.test) {
  console.log('ClrAddons sets to test: ', program.args);
  console.log('--------------');
 ...
 */
if (program.update) {
  console.log('ClrAddons sets to update: ', program.args);
  console.log('----------------');
...
function runGemini(config) {
  ...
  let server = shell.exec('node_modules/.bin/lite-server', { async: true });
  let status = shell.exec(
    'docker run --rm --name=clraddons_chrome -d -p 4444:4444 selenium/standalone-chrome@sha256:b899f16b6d963600ef6da8a8dd49e311146033ed66cb5af71eccb78ab378e19a'
  );
  let geminiStatus = 0;
  let screensDir = program.configuration ? `./gemini/${program.configuration}-screens` : './gemini/screens';
  if (status.code === 0) {
    waitForStartedSelenium();
    ...
    shell.exec('docker stop clraddons_chrome');
  } else {
    console.warn(
...
function waitForStartedSelenium() {
  let retries = 0;
  while (shell.exec('docker logs clraddons_chrome | grep "Selenium Server is up and running"').code !== 0) {
    retries++;
    if (retries < 40) {
