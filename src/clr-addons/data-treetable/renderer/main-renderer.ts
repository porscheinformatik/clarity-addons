...
import { DataTreeTableHeaderRenderer } from './header-renderer';
import { DataTreeTableRowRenderer } from './row-renderer';

@Directive({
  selector: 'clr-data-treetable',
  standalone: false,
})
export class DataTreeTableMainRenderer<T> implements AfterViewChecked, AfterContentInit {
  @ContentChildren(DataTreeTableHeaderRenderer) headers: QueryList<DataTreeTableHeaderRenderer>;
  @ContentChildren(DataTreeTableRowRenderer, { descendants: true })
  rows: QueryList<DataTreeTableRowRenderer>;
  @ContentChildren(ClrDataTreeTableColumn) columns: QueryList<ClrDataTreeTableColumn<T>>;
