...
// Test basic component - tests component logic without rendering the template
@Component({
  template: `
    <clr-summary-item
      [label]="label"
...
// Test host that uses ClrSummaryArea which properly renders templates
@Component({
  template: `
    <clr-summary-area>
      <clr-summary-item
  ...
  template: `
    <clr-summary-area>
      <clr-summary-item [label]="'Multi Value'" [showOnEmptyValue]="showOnEmptyValue">
        <clr-summary-item-value [value]="value1"></clr-summary-item-value>
        <clr-summary-item-value [value]="value2"></clr-summary-item-value>
      </clr-summary-item>
    </clr-summary-area>
  `,
  standalone: true,
...
describe('SummaryItemComponent', () => {
  ...
  describe('Edit config', () => {
    ...
    // Note: This test is moved to DOM rendering section since it requires template rendering for content detection
  ...
  // Tests that require rendered DOM use ClrSummaryArea as the parent
  describe('DOM rendering with ClrSummaryArea', () => {
    ...
    beforeEach(async () => {
      ...
      fixture = TestBed.createComponent(TestHostWithAreaComponent);
      hostComponent = fixture.componentInstance;
  ...
  describe('Custom element projection', () => {
    ...
    beforeEach(async () => {
      ...
      customFixture = TestBed.createComponent(CustomElementTestComponent);
    ...
    it('should show item when custom element is projected even with showOnEmptyValue=false', () => {
      const summaryItem = customFixture.debugElement.query(By.css('.summary-item'));
      expect(summaryItem.nativeElement.classList.contains('hidden')).toBe(false);
    });
  });

  describe('Multiple value children with ClrSummaryArea', () => {
    ...
    it('should not detect content when all value children have no meaningful content', () => {
      hostComponent.value1 = undefined;
      hostComponent.value2 = undefined;
      fixture.detectChanges();

      // Both children have no value, icon, or projected content - so no meaningful content
      expect(component.hasProjectedContent).toBe(false);
    });
    ...
    it('should not show parent placeholder when child with showOnEmptyValue=true exists', () => {
      ...
      // Both children are hidden because they have no meaningful content
      // The parent has showOnEmptyValue=true, so it shows its own placeholder
      const placeholders = fixture.debugElement.queryAll(By.css('.value-placeholder'));
      expect(placeholders.length).toBeGreaterThanOrEqual(1);
    });

    it('should hide value children with no content', () => {
      hostComponent.value1 = undefined;
      hostComponent.value2 = undefined;
      fixture.detectChanges();

      const valueComponents = fixture.debugElement.queryAll(By.directive(ClrSummaryItemValue));
      // Both children should be hidden since they have no meaningful content
      expect(valueComponents[0].nativeElement.classList.contains('hidden')).toBe(true);
      expect(valueComponents[1].nativeElement.classList.contains('hidden')).toBe(true);
    });
  });
});
